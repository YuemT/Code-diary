...
6. 异常处理（try...catch...finally）
    ...
    * 语法：
      		...
      		try{
      			//我们可以将有可能出现异常的代码放到try中                          //这样当代码在执行时，如果没有异常，则正常执行                          //如果出现异常，则代码不会执行，也不会将异常抛出
      		}catch(e){
      			//如果try中的语句出现了异常，则try不会执行，而是执行catch                          //我们可以将异常处理的代码编写catch来对异常进行处理                          //console.log("出错了~~~~");                          //catch中的e就是出现的异常信息
      		}finally{
